...
def currently_accesiable():
    crr_accessible_roll = 0
    for i in range(total_row):
        for j in range(total_col):
            if paper_rolls[i][j] and is_accessible(i, j):
                crr_accessible_roll += 1
                paper_rolls[i][j] = 0

    if crr_accessible_roll > 0:
        crr_accessible_roll = crr_accessible_roll + currently_accesiable()
    
    return crr_accessible_roll


total_accessible_roll = currently_accesiable()
